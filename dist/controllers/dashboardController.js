...
export const dashboard = asyncHandler(async (_req, res) => {
    ...
    const [users, activeUsers, citizenUsers, openDemands, resolvedDemands, lateDemands, events, surveys, notifications, usersByNeighborhood, demandEvolution, engagement, geolocatedUsers, geolocatedDemands, topParticipationNeighborhood, topComplaintNeighborhood, topDemandStreets, mostComplaintsNeighborhoods, activeNeighborhoods, recentDemands, posts, recentEvents, recentSurveys, recentNotifications, resolvedRecentDemands, mapUsers, mapDemands, mapEvents, heatmapUsers, heatmapDemands, topics] = await Promise.all([
        User.countDocuments(),
        User.countDocuments({
...
export const adoption = asyncHandler(async (_req, res) => {
    ...
    const activeActivityUserIds = await CitizenActivity.distinct("userId", { createdAt: { $gte: thirtyDaysAgo } });
    const [totalUsers, activeUsers, usersToday, usersThisMonth, usersPreviousMonth, usersWithCoordinates, topNeighborhoods, registrationsByMonth, neighborhoods, settings] = await Promise.all([
        User.countDocuments({ role: "CIDADAO" }),
        User.countDocuments({
            role: "CIDADAO",
            $or: [{ lastLoginAt: { $gte: thirtyDaysAgo } }, { _id: { $in: activeActivityUserIds } }]
...
function normalizeDemandEvolution(rows) {
    const byMonth = new Map();
    for (const row of rows) {
        const month = row._id.month;
        const current = byMonth.get(month) ?? { _id: month, abertas: 0, resolvidas: 0, total: 0 };
        current.total += row.total;
        if (row._id.status === "RESOLVIDO")
            current.resolvidas += row.total;
        else
            current.abertas += row.total;
...
async function calculateMostMentionedTopics() {
    ...
    for (const demand of demands) {
        ...
        for (const [topic, terms] of Object.entries(topicMap)) {
            ...
            if (matches)
                counts.set(topic, (counts.get(topic) ?? 0) + matches);
        }
    }
