...
function withFallbackCoordinates(payload, address) {
    const fallback = fallbackCoordinates(address);
    if (!fallback)
        return { ...payload, geocodingStatus: "failed", geocodingProvider: "openstreetmap-nominatim" };
    return {
        ...payload,
