...
export async function getConfirmedMapUsers() {
    ...
    const allActiveUsers = await User
        ...
        .lean();
    console.log("[ALL_ACTIVE_CITIZENS_FOR_MAP]", allActiveUsers.map(u => ({
        name: u.name,
        role: u.role,
        isActive: u.isActive,
        latitude: u.latitude,
    ...
    const confirmed = allActiveUsers.filter(u => {
        ...
        return (lat !== null && lat !== undefined && typeof lat === "number" && isFinite(lat) &&
            lng !== null && lng !== undefined && typeof lng === "number" && isFinite(lng));
    });
    console.log("[CONFIRMED_CITIZENS_FOR_MAP]", confirmed.map(u => ({
        name: u.name,
        role: u.role,
...
export function missingUserCoordinateQuery(extra = {}) {
    return {
