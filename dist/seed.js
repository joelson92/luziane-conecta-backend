...
for (const neighborhood of neighborhoods) {
    ...
    await Neighborhood.updateOne({ $or: [{ normalizedName }, { name: neighborhood.name }] }, { $set: { ...neighborhood, normalizedName, isActive: true } }, { upsert: true });
}
const centro = await Neighborhood.findOne({ normalizedName: normalizeNeighborhoodName("Centro") });
await Post.updateOne({ title: "Mensagem da Luziane" }, {
    $set: {
        category: "AVISOS",
