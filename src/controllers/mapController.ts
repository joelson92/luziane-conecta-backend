...
import { CITIZEN_ROLES, getCitizenGeoStats, getConfirmedMapUsers } from "../services/userGeoService.js";
...
export const mapUsers = asyncHandler(async (_req, res) => {
  ...
  const data = mapUsers_.map((user) => ({
    ...
    name: user.name,
    role: user.role,
    neighborhoodName: (user.neighborhoodName ?? (user as any).neighborhood ?? ""),
    city: user.city ?? "",
    state: user.state ?? "",
    street: (user as any).street ?? "",
    number: (user as any).number ?? "",
    community: (user as any).community ?? "",
...
export const mapSummary = asyncHandler(async (_req, res) => {
  ...
  for (const u of confirmedUsers) {
    const n = u.neighborhoodName || (u as any).neighborhood || "";
    if (n.trim()) neighborhoodNames.add(n.trim());
  }

  // Distinct municipalities
  const cityNames = new Set<string>();
  for (const u of confirmedUsers) {
    if (u.city?.trim()) cityNames.add(u.city.trim());
  }
...
export const mapNeighborhoods = asyncHandler(async (_req, res) => {
  ...
  const groups = new Map<string, { name: string; city: string }>();

  for (const user of users_) {
    const rawName = (user.neighborhoodName || (user as any).neighborhood || "").trim();
    if (!rawName) continue;
  ...
  const neighborhoods = Array.from(groups.values()).sort((a, b) =>
...
export const mapHeatmap = asyncHandler(async (req, res) => {
  ...
  const demandQuery: Record<string, any> = {
    "location.lat": { $type: "number" },
    "location.lng": { $type: "number" }
  };
  if (req.query.city) demandQuery.city = req.query.city;
  if (req.query.neighborhoodName) demandQuery.neighborhoodName = req.query.neighborhoodName;
  if (req.query.neighborhoodId) demandQuery.neighborhoodId = req.query.neighborhoodId;
...
export const mapDemands = asyncHandler(async (req, res) => {
  ...
  if (req.query.neighborhoodName) query.neighborhoodName = req.query.neighborhoodName;
  if (req.query.neighborhoodId) query.neighborhoodId = req.query.neighborhoodId;
...
export const mapUserStats = asyncHandler(async (req, res) => {
  ...
  if (req.query.city) extra.city = req.query.city;
  if (req.query.neighborhoodName) extra.neighborhoodName = req.query.neighborhoodName;
  if (req.query.neighborhoodId) extra.neighborhoodId = req.query.neighborhoodId;
